...
async function getAllowedRoleId() {
    const envRoleId = getAllowedRoleIdFromEnv();
    if (envRoleId) return envRoleId;

    const config = await loadDashboardConfig().catch(() => null);
    return config?.bot?.dashboardAllowedRoleId || "";
}
...
async function getOAuthConfig(req) {
    ...
    const config = {
        ...
        guildId: process.env.COMMUNITY_GUILD_ID || process.env.DISCORD_GUILD_ID || dashboardConfig?.bot?.communityGuildId || dashboardConfig?.bot?.guildId || "",
        allowedRoleId: getAllowedRoleIdFromEnv() || dashboardConfig?.bot?.dashboardAllowedRoleId || "",
