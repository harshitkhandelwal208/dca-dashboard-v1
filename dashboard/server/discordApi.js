...
function getConfiguredGuildIdsFromConfig(config) {
    return uniqueIds([
        config?.bot?.communityGuildId,
        config?.bot?.recruitmentGuildId,
        config?.bot?.guildId,
...
async function getConfiguredGuildIds() {
    ...
    return getConfiguredGuildIdsFromConfig(config);
}

async function getConfiguredGuildId() {
    return (await getConfiguredGuildIds())[0] || "";
}
...
async function getGuildMember(userId) {
    ...
    if (!guildIds.length) throw new Error("DISCORD_GUILD_ID is required.");

    for (const guildId of guildIds) {
        try {
            return await discordBotRequest(`/guilds/${guildId}/members/${userId}`);
        } catch (error) {
            lastError = error;
        }
    }

    throw lastError || new Error("Guild member lookup failed.");
}

async function getGuildMembers(userId) {
    const guildIds = await getConfiguredGuildIds();
    const members = [];
    const errors = [];
    ...
    for (const guildId of guildIds) {
        try {
            members.push({ guildId, member: await discordBotRequest(`/guilds/${guildId}/members/${userId}`) });
        } catch (error) {
