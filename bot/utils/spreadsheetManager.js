...
const LOCAL_IMAGE_ARTIFACT_RE = /\.(png|jpe?g|webp|gif|svg)$/i;
const GENERATED_IMAGE_OUTPUT_FIELDS = ["spreadsheetImagePath", "chartPath", "tableImagePath"];
...
function outputBaseDir(teamId, sessionId) {
    return path.join(spreadsheetDataDir(), safeFileName(teamId, "team"), safeFileName(sessionId, "session"));
}

function spreadsheetDataDir() {
    return path.join(__dirname, "..", "data", "spreadsheets");
}
...
function isLocalSpreadsheetImage(filePath) {
    if (!filePath || !LOCAL_IMAGE_ARTIFACT_RE.test(filePath)) return false;
    const root = path.resolve(spreadsheetDataDir());
    const resolved = path.resolve(filePath);
    return resolved === root || resolved.startsWith(`${root}${path.sep}`);
}

async function removeEmptyParents(filePath, stopDir = spreadsheetDataDir()) {
    ...
    while (current.startsWith(`${root}${path.sep}`)) {
        ...
        await fs.promises.rmdir(current).catch(() => null);
        current = path.dirname(current);
    }
}

async function cleanupGeneratedImageOutputs(outputs = {}) {
    let deleted = 0;
    const filePaths = GENERATED_IMAGE_OUTPUT_FIELDS
        .map(field => outputs[field])
        .filter(isLocalSpreadsheetImage);
    ...
    for (const filePath of [...new Set(filePaths)]) {
        ...
        await removeEmptyParents(filePath).catch(() => null);
    }

    return deleted;
}

async function cleanupLocalSpreadsheetImages(options = {}) {
    const root = spreadsheetDataDir();
    if (!fs.existsSync(root)) return { deletedFiles: 0, deletedDirs: 0 };

    const retentionDays = Math.max(1, Number(options.retentionDays || 7));
    const imageCutoff = Date.now() - retentionDays * 24 * 60 * 60 * 1000;
    const sourceCutoff = Date.now() - 24 * 60 * 60 * 1000;
    let deletedFiles = 0;
    let deletedDirs = 0;
    ...
    async function walk(dir) {
        ...
        for (const entry of entries) {
            ...
            if (entry.isDirectory()) {
                ...
                if (entry.name === "source" && stat && stat.mtimeMs < sourceCutoff) {
                    await fs.promises.rm(fullPath, { recursive: true, force: true });
                    deletedDirs += 1;
                    continue;
                }
                await walk(fullPath);
                await removeEmptyParents(fullPath, root).catch(() => null);
                continue;
            }

            if (!entry.isFile() || !LOCAL_IMAGE_ARTIFACT_RE.test(entry.name)) continue;
            const stat = await fs.promises.stat(fullPath).catch(() => null);
            if (!stat || stat.mtimeMs > imageCutoff) continue;
...
function postedAttachmentUrls(outputs = {}, messages = []) {
    const byName = new Map();
    for (const message of messages || []) {
        for (const attachment of message.attachments?.values?.() || []) {
            if (attachment.name && attachment.url) {
                byName.set(attachment.name, attachment.url);
            }
        }
    }

    const urls = {};
    for (const [pathField, urlField] of [
        ["fodsPath", "fodsUrl"],
        ["spreadsheetPath", "spreadsheetUrl"],
        ["spreadsheetImagePath", "spreadsheetImageUrl"],
        ["chartPath", "chartUrl"],
        ["filePath", "fileUrl"],
        ["tableImagePath", "tableImageUrl"]
    ]) {
        const filePath = outputs[pathField];
        if (!filePath) continue;
        const url = byName.get(path.basename(filePath));
        if (url) urls[urlField] = url;
    }
...
async function sendPeriodReport(client, teamConfig, period, options = {}) {
    ...
    await markReportEmitted(report, {
        ...
        channelId,
        messageId: message.id,
        attachmentUrls: Object.values(postedAttachmentUrls(report, [message]))
    });
    await cleanupGeneratedImageOutputs(report).catch(error => {
        console.error("Spreadsheet report image cleanup failed:", error.message);
    });
...
async function sendSessionOutput(client, session) {
    ...
    try {
        ...
        const attachmentUrls = postedAttachmentUrls(session.outputs, messages);
        if (Object.keys(attachmentUrls).length) {
            await updateSpreadsheetSession(session.id, {
                outputs: {
                    ...session.outputs,
                    ...attachmentUrls
                ...
                console.error("Spreadsheet output URL save failed:", error.message);
            });
        }
        if (teamConfig) {
            await postAutomaticReports(client, teamConfig, session);
        }
        await cleanupGeneratedImageOutputs(session.outputs).catch(error => {
            console.error("Spreadsheet image cleanup failed:", error.message);
        });
...
async function runSpreadsheetMaintenance(client, options = {}) {
    ...
    await cleanupLocalSpreadsheetImages({
        retentionDays: config.spreadsheets.imageRetentionDays || 7
    }).catch(error => {
        console.error("Spreadsheet image cleanup failed:", error.message);
    });
