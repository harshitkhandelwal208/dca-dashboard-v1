...
function cleanTeamName(value, ownTeamName = "") {
    let text = cleanText(value, "", 100)
        .replace(/\b(?:winner|team|score|points?)\b/gi, " ")
        .replace(/\s+/g, " ")
        .trim();
    const own = cleanText(ownTeamName, "", 80);
    const ownKey = normalizeKey(own);
    const numberTokens = text.match(/\b\d+\b/g) || [];
    const hasScoreFragments = (ownKey && normalizeKey(text).includes(ownKey)) ||
        /\b\d{1,3}[\s,.]\d{3}\b/.test(text) ||
        numberTokens.length >= 2;

    if (own) {
        text = text.replace(new RegExp(`\\b${escapeRegExp(own)}\\b`, "ig"), " ");
    }
    if (hasScoreFragments) {
        text = text.replace(/\b\d[\d\s,.]*\b/g, " ");
    }
    ...
    const pieces = text
        ...
        .filter(Boolean)
        .filter(part => normalizeKey(part) !== ownKey);

    if (pieces.length) {
        text = pieces[pieces.length - 1];
    }

    return cleanText(text.replace(/\s+/g, " "), "", 80);
}
...
function namesFromScoreLine(rawLine, ownTeamName) {
    ...
    return String(rawLine || "")
        .split(/\bvs\b|,|;/i)
        .map(part => cleanTeamName(part.replace(/\d[\d\s,.]*$/g, ""), ownTeamName))
...
function sessionOpponentTeams(session) {
    ...
    const explicit = teams
        .map(team => ({
            label: cleanTeamName(team?.label || team?.name || team?.teamName || "", ownTeamName),
    ...
    const fromPlayers = opponentPlayersForSession(session)
        .map(player => cleanTeamName(player.teamLabel || "", ownTeamName))
        .filter(label => {
