...
function buildReportModel(teamConfig, sessions, period, bounds, allSessions = sessions) {
    ...
    const weeklyRanking = period === "weekly";
    rows.sort((a, b) => weeklyRanking
    ...
    rows.forEach((row, index) => {
        const primary = weeklyRanking ? row.bluesKilled : row.totalScore;
        const secondary = weeklyRanking ? null : row.totalPoints;
        if (primary !== previousPrimary || secondary !== previousSecondary) rank = index + 1;
        row.rank = rank;
...
function reportPill(x, y, label, value, width = 112) {
    return `<g>
<rect x="${x}" y="${y}" width="${width}" height="24" fill="rgba(255,255,255,0.14)" stroke="rgba(255,255,255,0.24)" rx="12"/>
<text x="${x + 12}" y="${y + 16}" font-family="Arial" font-size="10" font-weight="700" fill="#d1fae5">${escapeXml(String(label).toUpperCase())}</text>
<text x="${x + width - 12}" y="${y + 17}" font-family="Arial" font-size="13" font-weight="700" text-anchor="end" fill="#ffffff">${escapeXml(value)}</text>
</g>`;
}

function eventTeamName(event) {
    const cleaned = (event.opponentTeams || [])
        .map(name => cleanTeamName(name, ""))
        .find(Boolean);
    return cleaned || "Opponent";
}

function weeklyReportRows(model) {
    const rows = model.rows.map(row => {
        const eventCells = row.events.map(event => {
            const attended = event.rank !== null;
            return {
                attended,
                value: attended ? event.bluesKilled : "",
                possible: attended ? event.possibleBlues : 0
            };
        });
        const total = eventCells.reduce((sum, event) => sum + (Number(event.value) || 0), 0);
        const max = eventCells.reduce((sum, event) => sum + (event.possible || 0), 0);
        return {
            ...row,
            eventCells,
            displayTotal: total,
            displayMax: max,
            displayPercent: blueKillPercent(total, max)
        };
    }).sort((a, b) =>
    ...
    rows.forEach((row, index) => {
        if (row.displayTotal !== previousTotal) rank = index + 1;
        row.displayRank = rank;
        previousTotal = row.displayTotal;
    });

    return rows;
}

function weeklyEventCellFill(eventCell, baseFill) {
    if (!eventCell.attended) return "#f8fafc";
    const value = Number(eventCell.value) || 0;
    const possible = Number(eventCell.possible) || 0;
    if (possible > 0 && value >= possible) return "#fef08a";
    const ratio = possible > 0 ? value / possible : 0;
    if (ratio >= 0.9) return "#bbf7d0";
    if (ratio >= 0.75) return "#dbeafe";
    if (ratio >= 0.5) return "#ede9fe";
    if (value > 0) return "#ffedd5";
    return baseFill;
}

function buildWeeklyReportTableSvg(model) {
    const rows = weeklyReportRows(model);
    const events = model.events || [];
    const eventWidth = events.length > 5 ? 68 : 78;
    const columns = [
        { id: "rank", label: "Rank", width: 72, align: "center", value: row => row.displayRank },
        { id: "name", label: "Driver", width: 250, align: "left", value: row => row.name },
        ...events.map((event, index) => ({
            id: `event-${index}`,
            label: eventTeamName(event),
            count: event.opponentCount || 0,
            width: eventWidth,
            align: "center",
            value: row => row.eventCells[index]?.value ?? "",
            cellFill: row => weeklyEventCellFill(row.eventCells[index] || {}, "")
        })),
        { id: "percent", label: "%Kill", width: 84, align: "center", value: row => row.displayMax ? `${row.displayPercent}%` : "" },
        { id: "total", label: "Total", width: 78, align: "center", value: row => row.displayTotal || "" },
        { id: "max", label: "Max", width: 76, align: "center", value: row => row.displayMax || "" },
        { id: "kab", label: "#KAB", width: 76, align: "center", value: row => row.kab || 0 }
    ];
    const margin = 24;
    ...
    const bodyStartY = titleHeight + groupHeight + headerHeight;
    const height = bodyStartY + rows.length * rowHeight + 34;
    const eventStartX = margin + columns[0].width + columns[1].width;
    const statsStartX = eventStartX + events.length * eventWidth;
    const statsWidth = columns.slice(2 + events.length).reduce((total, column) => total + column.width, 0);
    const pillX = Math.max(margin + 420, width - 306);
    const pills = [
        reportPill(pillX, 26, "Sessions", String(events.length), 128),
        reportPill(pillX + 138, 26, "Drivers", String(rows.length), 128)
    ].join("\n");

    let x = margin;
    const staticHeaders = columns.slice(0, 2).map(column => {
        const svg = `<rect x="${x}" y="${titleHeight}" width="${column.width}" height="${groupHeight + headerHeight}" fill="#1f2937" stroke="#111827"/>
<text x="${x + column.width / 2}" y="${titleHeight + 39}" font-family="Arial" font-size="17" font-weight="700" text-anchor="middle" fill="#ffffff">${escapeXml(column.label)}</text>`;
        x += column.width;
        return svg;
    }).join("\n");

    const eventHeaders = events.map((event, index) => {
        const columnX = eventStartX + index * eventWidth;
        const label = previewTextValue(eventTeamName(event), eventWidth > 72 ? 9 : 7);
        return `<rect x="${columnX}" y="${titleHeight}" width="${eventWidth}" height="${groupHeight}" fill="#bfdbfe" stroke="#1d4ed8"/>
<text x="${columnX + eventWidth / 2}" y="${titleHeight + 21}" font-family="Arial" font-size="15" font-weight="700" text-anchor="middle" fill="#111827">${escapeXml(label)}</text>
<rect x="${columnX}" y="${titleHeight + groupHeight}" width="${eventWidth}" height="${headerHeight}" fill="#eff6ff" stroke="#1d4ed8"/>
<text x="${columnX + eventWidth / 2}" y="${titleHeight + groupHeight + 22}" font-family="Arial" font-size="17" font-weight="700" text-anchor="middle" fill="#111827">${event.opponentCount || 0}</text>`;
    }).join("\n");

    const statHeaders = [
        `<rect x="${statsStartX}" y="${titleHeight}" width="${statsWidth}" height="${groupHeight}" fill="#fef3c7" stroke="#111827"/>
<text x="${statsStartX + statsWidth / 2}" y="${titleHeight + 21}" font-family="Arial" font-size="17" font-weight="700" text-anchor="middle" fill="#111827">Kill Stats</text>`
    ];
    let statX = statsStartX;
    for (const column of columns.slice(2 + events.length)) {
        const headerFill = column.id === "kab" ? "#0f7dba" : "#1f2937";
        statHeaders.push(`<rect x="${statX}" y="${titleHeight + groupHeight}" width="${column.width}" height="${headerHeight}" fill="${headerFill}" stroke="#111827"/>
<text x="${statX + column.width / 2}" y="${titleHeight + groupHeight + 22}" font-family="Arial" font-size="15" font-weight="700" text-anchor="middle" fill="#ffffff">${escapeXml(column.label)}</text>`);
        statX += column.width;
    }

    const body = rows.map((row, rowIndex) => {
        const y = bodyStartY + rowIndex * rowHeight;
        const baseFill = rowIndex % 2 === 0 ? "#ffffff" : "#dbeafe";
        let cellX = margin;
        return columns.map((column, columnIndex) => {
            const raw = column.value(row);
            const value = previewTextValue(raw, column.id === "name" ? 28 : 8);
            const fill = column.cellFill ? column.cellFill(row) || baseFill : column.id === "kab" && row.kab > 0 ? "#0f7dba" : baseFill;
            const color = column.id === "kab" && row.kab > 0 ? "#ffffff" : "#111827";
            const weight = columnIndex >= 2 || column.id === "rank" ? "700" : "400";
            const textAnchor = column.align === "center" ? "middle" : "start";
            const textX = column.align === "center" ? cellX + column.width / 2 : cellX + 9;
            const svg = `<rect x="${cellX}" y="${y}" width="${column.width}" height="${rowHeight}" fill="${fill}" stroke="#2563eb" stroke-width="0.75"/>
<text x="${textX}" y="${y + 21}" font-family="Arial" font-size="14" font-weight="${weight}" text-anchor="${textAnchor}" fill="${color}">${escapeXml(value)}</text>`;
            cellX += column.width;
            return svg;
        }).join("\n");
    }).join("\n");
...
<text x="${margin}" y="35" font-family="Arial" font-size="25" font-weight="700" fill="#ffffff">${escapeXml(`${model.teamName} ${model.period === "weekly" ? "Weekly" : "Monthly"} Report`)}</text>
<text x="${margin}" y="63" font-family="Arial" font-size="15" font-weight="600" fill="#d1fae5">${escapeXml(previewTextValue(model.periodLabel, 62))}</text>
${pills}
<text x="${width / 2}" y="116" font-family="Georgia, 'Times New Roman', serif" font-size="31" font-weight="700" text-anchor="middle" fill="#0f7dba">Team Event - ${escapeXml(previewTextValue(model.eventName, 42))}</text>
${staticHeaders}
${eventHeaders}
${statHeaders.join("\n")}
${body}
...
function buildReportTableSvg(model) {
    if (model.period === "weekly") {
        return buildWeeklyReportTableSvg(model);
    }
