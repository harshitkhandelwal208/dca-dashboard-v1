...
async function replyWithPeriodReport(interaction, teamConfig, period) {
    ...
    await cleanupLocalImages(report);
}
...
async function cleanupLocalImages(outputs) {
    await cleanupGeneratedImageOutputs(outputs).catch(error => {
        console.error("Spreadsheet command image cleanup failed:", error.message);
    });
}

async function rebuildIfMissing(interaction, session, paths = []) {
    if (!paths.length || paths.some(filePath => !filePath || !attachmentFor(filePath))) {
        return rebuildSpreadsheetSession(interaction.client, session.id);
    }
    return session;
}
    ...
    async execute(interaction) {
        ...
        if (subcommand === "status") {
            ...
            await interaction.editReply([
                ...
                `Gemini model: **${config.spreadsheets.geminiModel || process.env.GEMINI_FLASH_MODEL || "gemini-3.6-flash"}**`,
                ...
                `Local image retention: **${config.spreadsheets.imageRetentionDays || 7} day(s)**`,
        ...
        if (subcommand === "generate") {
            ...
            await cleanupLocalImages(processed.outputs);
            return;
        }
        ...
        if (subcommand === "file") {
            ...
            const readySession = await rebuildIfMissing(interaction, session, [session.outputs?.spreadsheetPath]);
            const files = filesForSession(readySession, false);
            await interaction.editReply(files.length
                ? { content: `Spreadsheet for \`${readySession.id}\`.`, files }
                : `No spreadsheet file exists for \`${readySession.id}\`. Rebuild it first.`
            );
            await cleanupLocalImages(readySession.outputs);
            return;
        }

        if (subcommand === "chart") {
            let session = await resolveSessionForTeam(
                teamConfig.id,
                interaction.options.getString("session_id", false),
                ["processed"]
            );
            ...
            session = await rebuildIfMissing(interaction, session, [session.outputs?.chartPath]);
            ...
            await cleanupLocalImages(session.outputs);
            return;
        }
        ...
        if (subcommand === "correct") {
            ...
            await cleanupLocalImages(corrected.outputs);
            return;
        }
        ...
        if (subcommand === "rebuild" || subcommand === "rebuild-event") {
            ...
            await cleanupLocalImages(rebuilt.outputs);
        }
    }
};
