...
function screenshotTypeChoices(option) {
    return option.addChoices(
        { name: "Driver's license", value: "license" },
        { name: "Team event scores", value: "event" }
    );
}

function imageOption(option, name = "image", required = true) {
    return option
        .setName(name)
        .setDescription(required ? "Image screenshot to save" : "Additional image screenshot to save")
...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addSubcommand(subcommand =>
            subcommand
                .setName("screenshot-list")
                .setDescription("List saved ticket screenshots and their indexes")
                .addStringOption(option =>
                    screenshotTypeChoices(option
                        .setName("type")
                        .setDescription("Which screenshot set to list")
                        .setRequired(true)
                    )
                )
        )
        .addSubcommand(subcommand =>
            subcommand
                .setName("screenshot-add")
                .setDescription("Add corrected screenshots to this ticket")
                .addStringOption(option =>
                    screenshotTypeChoices(option
                        .setName("type")
                        .setDescription("Which screenshot set to update")
                        .setRequired(true)
                    )
                )
                .addAttachmentOption(option => imageOption(option, "image", true))
                .addAttachmentOption(option => imageOption(option, "image_2", false))
                .addAttachmentOption(option => imageOption(option, "image_3", false))
                .addAttachmentOption(option => imageOption(option, "image_4", false))
                .addAttachmentOption(option => imageOption(option, "image_5", false))
        )
        .addSubcommand(subcommand =>
            subcommand
                .setName("screenshot-remove")
                .setDescription("Remove a saved screenshot from this ticket")
                .addStringOption(option =>
                    screenshotTypeChoices(option
                        .setName("type")
                        .setDescription("Which screenshot set to update")
                ...
                .addIntegerOption(option =>
                    option
                        .setName("index")
                        .setDescription("Screenshot index from /tickets screenshot-list")
                        .setMinValue(1)
                        .setRequired(true)
                )
        )
        .addSubcommand(subcommand =>
            subcommand
                .setName("screenshot-change")
                .setDescription("Replace a saved screenshot in this ticket")
                .addStringOption(option =>
                    screenshotTypeChoices(option
                        .setName("type")
                        .setDescription("Which screenshot set to update")
    ...
    async execute(interaction) {
        ...
        if (subcommand === "screenshot-list") {
            await listTicketScreenshots(interaction, interaction.options.getString("type", true));
            return;
        }

        if (subcommand === "screenshot-add") {
            const attachments = ["image", "image_2", "image_3", "image_4", "image_5"]
                .map(name => interaction.options.getAttachment(name, false))
                .filter(Boolean);
            await addTicketScreenshots(interaction, interaction.options.getString("type", true), attachments);
            return;
        }

        if (subcommand === "screenshot-remove") {
            await removeTicketScreenshot(
                interaction,
                interaction.options.getString("type", true),
                interaction.options.getInteger("index", true)
            );
            return;
        }

        if (subcommand === "screenshot-change") {
            await changeTicketScreenshot(
                interaction,
                interaction.options.getString("type", true),
                interaction.options.getInteger("index", true),
